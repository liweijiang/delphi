...
from scripts.collective_reasoning.NormBank.Ruler import *
...
class EnsembleRuler(Ruler):
    ...
    def optimize(self, vertices, edges):
        ...
        output_folder = "results/collective_reasoning/outputs/norm_bank/"
    ...
    def _print_results(self, class_label_preds, splits):
        for split in splits:
            ...
            for num_way in [2, 3]:
                ...
                for agreement_rate in ["all"]:  # [1, 0.8, 0.6, "ambiguous", "all"] , 1, "ambiguous"
    ...
    def main_preds2wcnf(self, class_label_preds, splits, is_print=True):
        ...
        data_path = f"results/collective_reasoning/outputs/norm_bank_class_label_preds_w_{w}_remove_dup_downweight_constituent.json"

        for event in tqdm(class_label_preds):
            class_label_preds = self.preds2wcnf(event, class_label_preds)
        save_json(data_path, class_label_preds)

        # class_label_preds = read_json(data_path)
