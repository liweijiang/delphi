...
class BaseRuler():
    def __init__(self):
        self.cache_name = "cache_norm_bank"
        self.data_type = "3000test"
    ...
    def _init_cache_data(self):
        self.paraphrases_cache = read_json(data_base_path + f"{self.cache_name}/paraphrases/{self.data_type}_paraphrases_filtered_by_nli_0.9.json")
        print(f"* Paraphrases cache loaded! ({len(self.paraphrases_cache)})")

        self.comet_cache = read_json(data_base_path + f"{self.cache_name}/comet/{self.data_type}_comet.json")
        print(f"* Comet cache loaded! ({len(self.comet_cache)})")

        self.delphi_cache = read_json(data_base_path + f"{self.cache_name}/delphi/{self.data_type}_delphi.json")
    ...
    def _init_gold_data(self):
        """
        Initialize gold data map and event maps
        """
        gold_data_path = data_base_path + f"{self.cache_name}/gold_data/{self.data_type}_gold_data.tsv"
        try:
            df_data_gold_label = pd.read_csv(gold_data_path, sep=",", low_memory=False)
        except:
            df_data_gold_label = pd.read_csv(gold_data_path, sep="\t", low_memory=False)

        df_data_gold_label = df_data_gold_label[~df_data_gold_label["clean_event"].isin(self.events_to_exclude)]
        df_data_gold_label = df_data_gold_label.drop_duplicates(subset=["clean_event"])
        df_data_gold_label["event_original"] = df_data_gold_label["event"]
        df_data_gold_label["event"] = df_data_gold_label["clean_event"]
        df_data_gold_label = df_data_gold_label.drop(columns=["clean_event"])
    ...
    def _init_raw_keywords_counts_map(self, is_save=False):
        ...
        """
        data_path = data_base_path + f"{self.cache_name}/keywords/{self.data_type}_keywords.json"
        if is_save:
            ...
            all_sequences_path = data_base_path + f"{self.cache_name}/all_sequences/{self.data_type}_all_sequences.json"
